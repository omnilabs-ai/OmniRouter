...
BENCHMARKS = {
    "MMLU": {
        ...
        "keywords": [
            "general knowledge", "multiple choice", "diverse subjects", "qa", "question answering",
            "trivia", "facts", "information", "broad knowledge", "general purpose", "multitask"
        ],
        "examples": [
...
def generate_benchmark_embeddings(output_file: str = "benchmark_embeddings.pkl", api_key: Optional[str] = None, cache_dir: Optional[str] = None):
    ...
    for benchmark_id, text in benchmark_texts.items():
        logger.info(f"Generating embedding for {benchmark_id}")
    ...
    logger.info(f"Saved embeddings for {len(embeddings)} benchmarks to {output_file}")
...
if __name__ == "__main__":
    ...
    root_env = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), args.env_file)
    ...
    if not api_key:
        logger.warning("No OpenAI API key found. Please ensure it's in your .env file as OPENAI_API_KEY=your_key or use --api_key.")
        sys.exit(1)
